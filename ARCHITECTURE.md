...
_Generated by the `architecture-doc` skill from `2069ec2` on 2026-08-18._
...
| API server | `server/src/` | Serves `/api/route`, `/health`, `/metrics` (JSON + Prometheus), `/client-error`, and the SPA fallback |
...
flowchart TB
    ...
    subgraph server["Node process — server/src"]
        index["index.js\nbinds PORT"]
        app["app.js\nExpress wiring · routes · health · metrics · client telemetry"]
        engine["routeEngine.js\ndeterministic routing, ValidationError"]
        metrics["metrics.js\ncounters + p50/p95/p99 · Prometheus exposition"]
    end
...
| `server/src/app.js` | Express wiring; `/health`, `/metrics`, `/client-error`; `ROUTE_DELAY_MS` injects upstream latency | Routes all traffic; the knob that reproduces regression B; logs JSON for structured parsing |
...
| `server/src/metrics.js` | In-process counters, percentiles over a 500-sample window; Prometheus exposition | The entire observability stack the policy needs — no external APM |
| `server/src/index.js` | Binds `PORT`; logs startup event | Cloud Run injects `PORT` and `RELEASE_VERSION`, `IMAGE_TAG` |
| Client-side telemetry | Browser posts errors to `/client-error` | Logs browser failures alongside server logs, correlatable by timestamp |
|
...
| **C** | A slow path through only one or two lanes (not in the gate's fixed sample) | green | green | Unseen by the gate's four fixed lanes; needs cross-diff analysis to catch |
|
A and B are green through every pre-merge check and deploy successfully. Only a
